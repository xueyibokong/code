...
Function.prototype.apply = function (context,args){ // 避免传入的是基本类型，使用object进行对象化 
    context = context ? Object(context) : window; context.fn = this; if (args) { return eval("context.fn("+args + ")"); } let argss = []; for (let i = 1;i<arguments.length;i++) { argss.push('arguments[' + i + ']'); } // es数组的结构 let r = eval("context.fn("+args + ")"); delete context.fn; return r; } fn1.apply('hellow',[1,2,3]);