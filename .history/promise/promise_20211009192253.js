...
/**
 * [x] 第四步 微任务（Micro task） 宏任务（Macro task）
 * 
 */
...
class Promise {
    constructor(callback) {
        ...
        this.rejectCb = () => {};
        this.cbReturn = undefined;
        callback(Promise._reslove.bind(this), Promise._reject.bind(this), Promise._finally.bind(this))
    }
    static _reslove(/** 形参 */res) {
        this.PromiseState = 'resolve';
        this.PromiseResult = res;
            ...
            for(let i = 0; i < len; i++) {
                ...
                if (promise.constructor !== Promise) {
                    ...
                    promise.then(res => {
                        ...
                        index++;
                        result.push({ status: 'fulfilled', reason: err });
                        if (index === len) {
                            finally(result);
                        }
                    })
